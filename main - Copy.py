...
class VideoStreamManager:
    ...
    def _reset_tracker(self):
        if self.debug_tracker and self.tracker_active:
            print(f"[TRACKER] Reset/Lost at frame {self.frame_count}")
        self.tracker = None
        self.tracker_bbox = None
        self.tracker_active = False
    ...
    def _initialize_tracker(self, frame, bbox):
        ...
        try:
            ...
            ok = self.tracker.init(frame, (int(x1), int(y1), width, height))
            if ok:
                ...
                self._reset_tracker()
                if self.debug_tracker:
                    print(f"[TRACKER] Init failed at frame {self.frame_count}")
        except Exception as e:
    ...
    def _update_tracker(self, frame):
        ...
        try:
            ...
            if ok:
                ...
                self.tracker_lost = False
                if self.debug_tracker:
                    print(f"[TRACKER] Updated at frame {self.frame_count}, bbox=({x}, {y}, {x+w}, {y+h})")
                return self.tracker_bbox
            else:
                if self.debug_tracker:
                    print(f"[TRACKER] Update failed at frame {self.frame_count}, resetting")
        except Exception as e:
    ...
    def process_frame(self):
        ...
        #main mdl note to self - this is specilized fire / smoke classification.
        ...
        #2nd mdl note to self - this is general classification.
        ...
        run_face_recognition = should_run_face_recognition(
            self.frame_count,
            process_interval,
            self.tracker_active,
            self.tracker_lost,
        ...
        if self.debug_tracker and self.frame_count % 30 == 0:
            print(f"[FRAME {self.frame_count}] face_rec={run_face_recognition}, tracker_active={self.tracker_active}, tracker_lost={self.tracker_lost}")

        if self.settings_cache.get('face_recognition_enabled', True) and run_face_recognition:
            ...
            if human_boxes:
                best_box = max(human_boxes, key=lambda item: item[4])
                self._initialize_tracker(frame, (best_box[0], best_box[1], best_box[2], best_box[3]))
            else:
                self._reset_tracker()
        elif self.tracker_active:
            tracked_box = self._update_tracker(frame)
            if tracked_box is None and human_boxes:
                best_box = max(human_boxes, key=lambda item: item[4])
                self._initialize_tracker(frame, (best_box[0], best_box[1], best_box[2], best_box[3]))
        ...
        if self.tracker_bbox:
            ...
            cv2.putText(annotated_frame, "KCF", (x1, max(0, y1 - 8)), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 0, 255), 2)
...
def admin_dashboard():
    ...
    for event in EventLog.query.order_by(EventLog.timestamp.desc()).limit(5).all():
        ...
        if 'critical' in event_type or 'alert' in event_type or 'unknown' in event_type or 'error' in event_type:
            dot_type = 'danger'
        elif 'login' in event_type or 'access granted' in event_type or 'recognized' in event_type or 'person' in event_type:
            dot_type = 'success'
